...
# from postfix import topostfix, evaluate_postfix
from postfixnyoba import infix_to_postfix, list_operator
...
def evaluate(postfix_expression, var):
    ...
    for t in postfix_expression:
        if t.isalnum():
            if t in var:
                stack.append(var[t])
            else:
                stack.append(int(t))
        elif list_operator(t):
            opand2 = stack.pop()
            opand1 = stack.pop()
            if t == '+':
                stack.append(opand1 + opand2)
            elif t == '-':
                stack.append(opand1 - opand2)
            elif t == '*':
                stack.append(opand1 * opand2)
...
def execute():
    ...
    while j < len(line):
        ...
        if len(sub_part) == 2: 
            ...
            expression_2 = infix_to_postfix(right_part)
            result_hasil_eval = evaluate(expression_2, var)
        ...
        if text.find('jika') != -1:
            ...
            if len (sub_part) == 4:
                ...
                if code_moresubpart == '=': 
                    ...
                    expression_l5= infix_to_postfix(change)
                    renew = evaluate(expression_l5, var)
                    var[left_part] = renew
        ...
        if text.find('cetak') != -1:
            ...
            try:
                result = evaluate(infix_to_postfix(command_right), var)
            ...
            if len(sub_part) == 3:
                ...
                expression_3 = infix_to_postfix(exp_to_eval)
                hasil_eval = evaluate(expression_3, var)
            ...
            if len (sub_part) == 5:
                ...
                # print("exp to eval", exp_to_eval)
                expression_5 = infix_to_postfix(exp_to_eval)
                hasil_eval = evaluate(expression_5, var)
...
output_text = tk.Text(root, height=20, width=120, state="disabled")
output_text.pack()
